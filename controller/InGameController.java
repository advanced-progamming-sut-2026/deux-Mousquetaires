...
class InGameController {
    ...
    void begin(GameSession session) {
        ...
        app.view.info("The level begins! Time only moves when you advance it:" + " advance time -t <n> ticks (10 ticks = 1 second). Type 'help' for commands.");
        app.view.info(session.showMap());
    }

    void handle(String line) {
        if (session == null) {
            app.view.error("No level is running.");
            return;}
        ...
        if (line.equals("menu show current")) {
            app.view.menuPath(MenuContext.IN_GAME.getDisplayName());
            return;}
    ...
    private void printHelp() {
        app.view.info("Commands: advance time -t <n> ticks | collect sun -l (x, y)"
                + " | show sun amount | plant plant -t <type> -l (x, y)"
                + " | pluck plant -l (x, y) | feed plant -l (x, y)"
                + "\n| show map | show plants status | show tile status -l (x, y)"
                + " | zombies info | show conveyor | start zombie waves"
                + "\n| cheat add -n <count> suns | cheat add-plant-food | cheat remove-cooldown"
                + " | cheat spawn-zombie -t <type> -l <x, y> | release the nuke | menu exit (forfeit)");
    }
    ...
    private void route(String line) {
        ...
        if (line.startsWith("advance time")) {
            ...
            if (ticks == null || ticks <= 0) {
                app.view.error("Usage: advance time -t <n> ticks");
                return;}
        ...
        if (line.startsWith("collect sun")) {
            ...
            if (at == null) {
                app.view.error("Usage: collect sun -l (x, y)");
                return;}

            app.view.info(session.collectSun(at[0], at[1]));
            return;}

        if (line.equals("show sun amount")) {
            app.view.info(session.showSunAmount());
            return;}
        ...
        if (line.startsWith("plant plant")) {
            ...
            if (type == null || at == null) {
                app.view.error("Usage: plant plant -t <type> -l (x, y)");
                return;}

            app.view.info(session.plantPlant(type, at[0], at[1]));
            return;}

        if (line.startsWith("pluck plant")) {
            int[] at = Args.location(args.get("l", line));
            if (at == null) {
                app.view.error("Usage: pluck plant -l (x, y)");
                return;}
            app.view.info(session.pluckPlant(at[0], at[1]));
            return;}

        if (line.startsWith("feed plant")) {
            int[] at = Args.location(args.get("l", line));
            if (at == null) {
                app.view.error("Usage: feed plant -l (x, y)");
                return;}
            app.view.info(session.feedPlant(at[0], at[1]));
            return;}
        ...
        if (line.equals("show plants status")) {
            app.view.info(session.showPlantsStatus());
            return;}

        if (line.startsWith("show tile status")) {
            int[] at = Args.location(args.get("l", line));
            if (at == null) {
                app.view.error("Usage: show tile status -l (x, y)");
                return;}
            app.view.info(session.showTileStatus(at[0], at[1]));
            return;}

        if (line.equals("zombies info")) {
            app.view.showLawnZombies(session.getZombiesOnLawn());
            return;}

        if (line.equals("show conveyor")) {
            app.view.info("Conveyor belt: " + session.getConveyorBelt());
            return;}

        if (line.equals("start zombie waves")) {
            app.view.info(session.startZombieWaves());
            return;}

        if (line.startsWith("cheat add -n") && line.endsWith("suns")) {
            Integer count = args.getInt("n");
            if (count == null || count <= 0) {
                app.view.error("Usage: cheat add -n <count> suns");
                return;}
        ...
        if (line.startsWith("cheat spawn-zombie")) {
            ...
            int[] at = Args.location(args.get("l"));
            if (type == null || at == null) {
                app.view.error("Usage: cheat spawn-zombie -t <type> -l <x, y>");
                return;}
            app.view.info(session.cheatSpawnZombie(type, at[0], at[1]));
            return;}

        if (line.equals("release the nuke")) {
            session.releaseNuke();
            return;}
        app.view.error("Unknown command. Type 'help' for the in-game commands.");
    }

    void forfeit() {
        app.view.info("You retreat from the lawn. The level does not count.");
        session = null;
        app.menuStack.pop();
    ...
    private void finish() {
        ...
        if (finished.isWon()) {
            ...
            app.view.success("LEVEL CLEARED! Rewards: " + coins + " coins, " + gems + " gems.");
        ...
        else app.view.info("Level failed. " + finished.getEndMessage());

        if (spec.isScored()) {
            app.currentUser.recordGamePlayed(finished.getMooPoints());
            app.view.info("Scored game finished with " + finished.getMooPoints()
                    + " MooPoints (your best: " + app.currentUser.getBestMooPoints() + ").");
        }
