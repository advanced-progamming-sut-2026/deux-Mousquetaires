...
public class ShopService {
    ...
    public void printCatalog(User user) {
        System.out.println("=== Shop – permanent items ===");
        System.out.println("  [1] pot            – " + POT_PRICE_COINS + " coins  (you own " + user.getPots() + "/" + User.MAX_POTS + ")");
        System.out.println("  [2] plant-food     – " + PLANT_FOOD_PRICE_GEMS + " gems   (you store " + user.getPlantFoods() + "/" + User.MAX_PLANT_FOOD + ")");
        System.out.println("  [3] random-packets – " + RANDOM_BUNDLE_PRICE_COINS + " coins  (" + RANDOM_BUNDLE_PACKETS + " seed packets, random unlocked plant)");
        System.out.println("  [4] chosen-packets – " + CHOSEN_BUNDLE_PRICE_GEMS + " gems   (" + CHOSEN_BUNDLE_PACKETS + " seed packets, use -t <plant>)");
        System.out.println("  [5] coins          – " + CONVERSION_PRICE_GEMS + " gems   (converted into " + CONVERSION_COINS + " coins)");
        System.out.println("Use: shop buy -i <item_id> -n <count> [-t <plant_type>]");
    }

    public void printDailyOffer(User user) {
        refreshDailyOffer(user);
        System.out.println("=== Daily offer (refreshes at midnight) ===");
        System.out.println("  [6] daily – " + DAILY_PACKETS + " seed packets of " + dailyOfferPlant + " for " + DAILY_DISCOUNT_PRICE + " coins (20% off " + DAILY_BASE_PRICE + ")");
        if (hasBoughtDailyToday(user)) System.out.println("  Already purchased today – come back tomorrow!");
    }
