...
public class Beghouled extends MiniGame {
    ...
    public void start() {
        ...
        fillBoard();
        System.out.println("=== Beghouled (level " + level + ") ===");
        System.out.println("Match 3+ plants to earn sun. Reach " + target + " sun to win!");
        System.out.println("Upgrades: repeater(500) gatling(1500) tallnut(500)" + " fume(250) melon(1000) winter(750)");
        System.out.println("Commands: swap (x1, y1) (x2, y2) | upgrade <name> | board | status | quit");
        printBoard();
    }
    ...
    public void handle(String line) {
        ...
        if (trimmed.startsWith("swap")) {
            java.util.regex.Matcher matcher = java.util.regex.Pattern
                    ...
                    .matcher(trimmed);
            if (!matcher.matches()) {
                System.out.println("Usage: swap (x1, y1) (x2, y2)   with x=1.." + COLS + ", y=1.." + ROWS);
                return;}
        ...
        switch (parts[0]) {
            case "upgrade":
                if (parts.length < 2) {
                    System.out.println("Usage: upgrade <repeater|gatling|tallnut|fume|melon|winter>");
                    return;}
            ...
            case "board":
                ...
                return;
            case "status":
                System.out.println("Sun: " + sun + "/" + target + " | Swaps made: " + swaps);
                return;
            case "quit":
    ...
    private void swap(int x1, int y1, int x2, int y2) {
        if (!inBounds(x1, y1) || !inBounds(x2, y2)) {
            System.out.println("Coordinates must be x=1.." + COLS + ", y=1.." + ROWS + ".");
            return;
        }
        if (Math.abs(x1 - x2) + Math.abs(y1 - y2) != 1) {
            System.out.println("You can only swap two adjacent plants.");
            return;
        }
        int row1 = y1 - 1,col1 = x1 - 1,row2 = y2 - 1,col2 = x2 - 1;
        if (board[row1][col1] == '#' || board[row2][col2] == '#') {
            System.out.println("You cannot swap into a crater.");
            return;
        }
        doSwap(row1, col1, row2, col2);
        if (!anyMatch()) {
            doSwap(row1, col1, row2, col2);
            System.out.println("That swap makes no match - the plants shuffle back.");
            return;
        }
        swaps++;
        resolveMatches();
        ...
        if (!anyPossibleMove()) {
            System.out.println("No moves left - the board reshuffles itself!");
            fillBoard();}
    ...
    private void resolveMatches() {
        ...
        while (true) {
            ...
            sun += gained;
            System.out.println("Match x" + combo + ": " + count + " plants -> +" + gained + " sun (total " + sun + ")");
            for (int r = 0; r < ROWS; r++) {
    ...
    private void upgrade(String name) {
        switch (name) {
            ...
            case "winter":
                ...
                return;
            default:
                System.out.println("Unknown upgrade. Options: repeater gatling tallnut fume melon winter");
        }
    }

    private void buyUpgrade(int cost, boolean prerequisite, Runnable apply, String message) {
        if (!prerequisite) {
            System.out.println("You do not meet the prerequisite for that upgrade yet.");
            return;}
        if (sun < cost) {
        ...
        sun -= cost;
        apply.run();
        System.out.println(message + " Sun left: " + sun);
        printBoard();
    }
