...
class ZombieEngine {
    ...
    private void handleContact(ActiveZombie activeZombie, PlantedUnit plant) {
        ...
        switch (type) {
            case EXPLORER:
                if (activeZombie.torchLit) {
                    System.out.println("The Explorer's torch burns " + plant.getType() + " at (" + plant.col + ", " + plant.row + ") to ash!");
            ...
            case SQUASH_ZOMBIE:
                if (!activeZombie.crushedPlant) {
                    ...
                    System.out.println("The Squash zombie crushes " + plant.getType() + "!");
                    session.destroyPlant(plant);
                    return;
                }
                break;
            case JALAPENO_ZOMBIE:
                System.out.println("The Jalapeno zombie explodes, scorching lane " + activeZombie.row + "!");
            ...
            case TROGLOBITE:
            case PIANIST:
                System.out.println("The " + type + " crushes " + plant.getType() + " flat!");
                session.destroyPlant(plant);
                return;
            case PEASHOOTER_ZOMBIE:
                if (activeZombie.abilityTimer <= 0) {
                    activeZombie.abilityTimer = 3 * GameSession.TICKS_PER_SECOND;
                    System.out.println("The Peashooter zombie shoots " + plant.getType() + "!");
        ...
        if (plant.getType() == PlantType.GARLIC && activeZombie.laneSwitchCooldown <= 0) {
            ...
            System.out.println("Yuck, garlic! The zombie staggers into lane " + activeZombie.row + ".");
            return;
        }
    ...
    private void handleIceSlip(ActiveZombie activeZombie) {
        ...
        if (cell != activeZombie.lastCellSeen) {
            ...
            if (type == TerrainType.ICE_SLIP_UP && activeZombie.row > 1) {
                activeZombie.row--;
                System.out.println("A zombie slips on the ice up to lane " + activeZombie.row + "!");
            }
            else if (type == TerrainType.ICE_SLIP_DOWN && activeZombie.row < GameSession.ROWS) {
                activeZombie.row++;
                System.out.println("A zombie slips on the ice down to lane " + activeZombie.row + "!");
            }
        }
    }
    ...
    private void reachHouse(ActiveZombie activeZombie) {
        ...
        if (session.mowers[row]) {
            ...
            for (ActiveZombie victim : new ArrayList<>(session.zombies)) {
                if (victim.row == row) {
                    System.out.println("  - " + victim.zombie.getType() + " at (" + victim.cellCol() + ", " + row + ")");
        ...
        switch (activeZombie.zombie.getType()) {
            case GARGANTUAR:
                if (!activeZombie.abilityUsed && activeZombie.zombie.getHp() <= activeZombie.zombie.getMaxHp() / 2) {
                    ...
                    System.out.println("The Gargantuar hurls its Imp to column 3!");
            ...
            case RA:
                if (activeZombie.abilityTimer <= 0) {
                    ...
                    if (session.sunBank >= 25) {
                        ...
                        System.out.println("Zombie Ra absorbs 25 sun! You now have " + session.sunBank + " sun.");
                    }
                }
                break;
            case TURQUOISE_SKULL:
                if (activeZombie.abilityTimer <= 0) {
                    ...
                    if (session.sunBank >= 25) {
                        session.sunBank -= 25;
                        activeZombie.sunStolen += 25;
                        System.out.println("The Turquoise Skull zombie drains 25 sun!");
                        if (activeZombie.sunStolen >= 75) {
                            activeZombie.sunStolen = 0;
                            System.out.println("The Turquoise Skull fires a sun-powered laser down lane " + activeZombie.row + "!");
                            for (PlantedUnit unit : new ArrayList<>(session.plants)) {
            ...
            case PROSPECTOR:
                if (!activeZombie.abilityUsed && activeZombie.col <= 5) {
                    ...
                    System.out.println("The Prospector's dynamite blasts him behind your defenses!" + " He now walks the lane in reverse.");
                }
                break;
            case PIANIST:
                if (activeZombie.abilityTimer <= 0) {
                    ...
                    if (session.rng.nextInt(100) < 30) {
                        System.out.println("The Pianist plays a wild tune - zombies switch lanes!");
            ...
            case ARCADE:
                if (activeZombie.abilityTimer <= 0) {
                    activeZombie.abilityTimer = 10 * GameSession.TICKS_PER_SECOND;
                    System.out.println("The Arcade machine spits out a fresh zombie!");
    ...
    private void raiseTombs() {
        ...
        while (raised < 2 && guard++ < 50) {
            ...
            if (session.terrain[y][x] == TerrainType.GRASS && session.findPlant(x, y) == null) {
                ...
                raised++;
                System.out.println("The Tombraiser raises a tombstone at (" + x + ", " + y + ")!");
            }
        }
    }
    ...
    private void throwSnowball(ActiveZombie activeZombie) {
        ...
        target.snowballHits++;
        System.out.println("The Hunter pelts " + target.getType() + " with a snowball (" + target.snowballHits + "/3)!");
        if (target.snowballHits >= 3) {
            target.frozen = true;
            System.out.println(target.getType() + " at (" + target.col + ", " + target.row + ") is frozen solid! Thaw it with fire.");
        }
    }
    ...
    private void bindPlant(ActiveZombie activeZombie) {
        for (PlantedUnit unit : session.plants) {
            if (unit.row == activeZombie.row && Math.abs(unit.col - activeZombie.col) <= 3 && !session.octopusBindings.containsValue(unit)) {
                ...
                System.out.println("The Octopus binds " + unit.getType() + " at (" + unit.col + ", " + unit.row + ")! It is helpless until the Octopus dies.");
                return;
            }
        }
    }
    ...
    private void curseFurthestPlant(ActiveZombie activeZombie) {
        ...
        System.out.println("The Wizard turns " + target.getType() + " at (" + target.col + ", " + target.row + ") into a harmless cat!");
    }
    ...
    private void knightABasic(ActiveZombie az) {
        for (ActiveZombie other : new ArrayList<>(session.zombies)) {
            if (other.zombie.getType() == ZombieType.BASIC) {
                System.out.println("The King dubs a Basic zombie - arise, Knight!");
    ...
    private void hookFrontPlant(ActiveZombie az) {
        ...
        if (target != null) {
            System.out.println("The Fisherman hooks " + target.getType() + " at (" + target.col + ", " + target.row + ") and drags it away!");
        ...
        if (cursed != null) {
            for (PlantedUnit unit : cursed) {
                if (session.plants.contains(unit)) {
                    ...
                    System.out.println(unit.getType() + " at (" + unit.col + ", " + unit.row + ") turns back from a cat!");
                }
            }
        }
    }
